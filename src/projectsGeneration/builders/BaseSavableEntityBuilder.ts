...
import {Storage, StorageType, isStorageClickHouseOnly, isStorageElasticOnly, isStorageExternalSearch} from './storage'
...
abstract class BaseSavableEntityBuilder extends BaseBuilder implements MethodsModelsHolder {
  ...
  searchEnabled = true
  protected _storage: StorageType = Storage.POSTGRES
  ...
  setStorage(storage: StorageType) {
    this._storage = storage
    this.externalSearchName = undefined

    if (isStorageExternalSearch(storage)) {
      this.getKey().setType('string').setAutoGenerated(true)
      this.externalSearchName = `external${pascal(this.name)}SearchTrackings`
      this.setSearchEnabled()
    } else if (isStorageElasticOnly(storage) || isStorageClickHouseOnly(storage)) {
      this.getKey().setType('string').setAutoGenerated(true)
    }
