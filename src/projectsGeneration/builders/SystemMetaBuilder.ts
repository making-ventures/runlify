...
export const defaultConfigVar: Omit<ConfigVar, 'name'> = {
  needFor: '',
  default: '',
...
class SystemMetaBuilder {
  ...
  constructor(
    ...
    this.addConfigVar('logs.format', 'plain', 'Формат логов (plain | json)')
    ...
    this.addConfigVar('kafka.enable', 'false', 'По умолчанию кафка не используется. true чтобы работала.');
