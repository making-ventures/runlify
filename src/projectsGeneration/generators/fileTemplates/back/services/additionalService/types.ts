...
${service.methods.filter(m => m.exportedToApi).map(m => `  ${m.name}: (${m.argsModel.fields.length ? `args: ${pascalCase(m.name)}Args` : ''}) => Promise<${m.returnModel.fields.length ? pascalCase(m.name) : 'void'}>`).join('\n')}
}` : '{}'}
