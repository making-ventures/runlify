...
export const configTmpl = ({
  ...
  if (isDocument) {
    ...
    registries = document.registries.map((registry) => `${singular(registry)}: StrictCreate${pascalSingular(registry)}Args[];`).map(pad(2)).join('\n');
...
import {${configType}} from '../types';${registriesImports}
${options.skipWarningThisIsGenerated ? '' : `
// ${generatedWarning}`}

export interface ${pascalPlural(entity.name)}Types {
  AutodefinableKeys: ${
  ...
  RegistryEntries: {${registries ? '\n' + registries + '\n  ' : ''}};`: ''}
}

const config: ${configType}<${pascalPlural(entity.name)}Types['AutodefinableKeys']> = {
